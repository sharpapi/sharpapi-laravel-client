<?php

/** @noinspection PhpUnused */

declare(strict_types=1);

namespace SharpAPI\SharpApiService\Enums;

use Kongulov\Traits\InteractWithEnum;

/**
 * @api
 */
class SharpApiLanguages
{
    use InteractWithEnum;

    public const AFRIKAANS = 'Afrikaans';

    public const ALBANIAN = 'Albanian';

    public const AMHARIC = 'Amharic';

    public const ARABIC = 'Arabic';

    public const ARMENIAN = 'Armenian';

    public const AZERBAIJANI = 'Azerbaijani';

    public const BENGALI = 'Bengali';

    public const BOSNIAN = 'Bosnian';

    public const BULGARIAN = 'Bulgarian';

    public const BURMESE = 'Burmese';

    public const CATALAN = 'Catalan';

    public const CHINESE_SIMPLIFIED = 'Chinese (Simplified)';

    public const CHINESE_TRADITIONAL = 'Chinese (Traditional)';

    public const CROATIAN = 'Croatian';

    public const CZECH = 'Czech';

    public const DANISH = 'Danish';

    public const DARI = 'Dari';

    public const DUTCH = 'Dutch';

    public const ENGLISH = 'English';

    public const ESTONIAN = 'Estonian';

    public const FINNISH = 'Finnish';

    public const FRENCH = 'French';

    public const GEORGIAN = 'Georgian';

    public const GERMAN = 'German';

    public const GREEK = 'Greek';

    public const GUJARATI = 'Gujarati';

    public const HEBREW = 'Hebrew';

    public const HINDI = 'Hindi';

    public const HUNGARIAN = 'Hungarian';

    public const ICELANDIC = 'Icelandic';

    public const INDONESIAN = 'Indonesian';

    public const ITALIAN = 'Italian';

    public const JAPANESE = 'Japanese';

    public const JAVANESE = 'Javanese';

    public const KANNADA = 'Kannada';

    public const KAZAKH = 'Kazakh';

    public const KINYARWANDA = 'Kinyarwanda';

    public const KOREAN = 'Korean';

    public const KURDISH_SORANI = 'Kurdish (Sorani)';

    public const KYRGYZ = 'Kyrgyz';

    public const LAO = 'Lao';

    public const LATVIAN = 'Latvian';

    public const LITHUANIAN = 'Lithuanian';

    public const MACEDONIAN = 'Macedonian';

    public const MALAY = 'Malay';

    public const MALAYALAM = 'Malayalam';

    public const MARATHI = 'Marathi';

    public const MONGOLIAN = 'Mongolian';

    public const NEPALI = 'Nepali';

    public const NORWEGIAN = 'Norwegian';

    public const ORIYA = 'Oriya';

    public const PASHTO = 'Pashto';

    public const PERSIAN = 'Persian';

    public const POLISH = 'Polish';

    public const PORTUGUESE = 'Portuguese';

    public const PUNJABI = 'Punjabi';

    public const ROMANIAN = 'Romanian';

    public const RUSSIAN = 'Russian';

    public const SERBIAN = 'Serbian';

    public const SESOTHO = 'Sesotho';

    public const SINHALA = 'Sinhala';

    public const SLOVAK = 'Slovak';

    public const SLOVENIAN = 'Slovenian';

    public const SOMALI = 'Somali';

    public const SPANISH = 'Spanish';

    public const SWAHILI = 'Swahili';

    public const SWEDISH = 'Swedish';

    public const TAGALOG = 'Tagalog';

    public const TAJIK = 'Tajik';

    public const TAMIL = 'Tamil';

    public const TELUGU = 'Telugu';

    public const THAI = 'Thai';

    public const TURKISH = 'Turkish';

    public const UKRAINIAN = 'Ukrainian';

    public const URDU = 'Urdu';

    public const UZBEK = 'Uzbek';

    public const VIETNAMESE = 'Vietnamese';

    public const XHOSA = 'Xhosa';

    public const YORUBA = 'Yoruba';

    public const ZULU = 'Zulu';
}
